...
int main()
{
    ...
    mpz_t *subCounter = create_bigint_with_value(10000000);
    mpz_t *modValue = create_bigint_with_value(1);
    mpz_mul_2exp(*modValue, *modValue, 32); // 2^32
    mpz_t *shift = create_bigint_with_value(28);

    mpz_t *one = create_bigint_with_value(1);
    mpz_t *zero = create_bigint_with_value(0);

    mpz_mul_2exp(*x0, *x0, 28);

    while (is_gt(subCounter, zero))
    {
        sub(x0, x0, operand);

        sub(subCounter, subCounter, one);
    }
